...
---
1. 不要覆写静态方法，但可以隐藏。而且，不能用override注解。建议：不要通过实例对象访问静态方法或静态属性，这不是一个好习惯。because:实例对象有两个类型：表面类型（Apparent Type）和实际类型（Actual Type）
   + 比如：Base base=new Sub（）；变量base的表面类型是Base，实际类型是Sub。

     对于非静态方法，它是根据对象的实际类型来执行的，也就是执行了Sub类中的doAnything方法。

     对于静态方法，如果是通过对象调用静态方法，JVM则会通过对象的表面类型查找到静态方法的入口，继而执行之
2. 构造函数尽量简化
